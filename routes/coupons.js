...
router.post('/', async (req, res) => {
  ...
  await db.prepare('INSERT INTO coupons (code, discount_type, discount_value, min_order, max_uses, expires_at, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)').run(code, discount_type, discount_value, min_order, max_uses, expires_at || null, b.is_active !== '0' ? 1 : 0);
  await logActivity(db, req.session.adminId, req.session.adminUsername, 'إضافة كوبون', code);
...
router.post('/update/:id', async (req, res) => {
  ...
  const max_uses = parseInt(b.max_uses, 10) || 0;
  const expires_at = b.expires_at || null;
  await db.prepare('UPDATE coupons SET discount_type = ?, discount_value = ?, min_order = ?, max_uses = ?, expires_at = ?, is_active = ? WHERE id = ?').run(discount_type, discount_value, min_order, max_uses, expires_at || null, b.is_active !== '0' ? 1 : 0, id);
  await logActivity(db, req.session.adminId, req.session.adminUsername, 'تعديل كوبون', b.code || id);
  res.redirect('/admin/coupons');
});
...
router.post('/delete/:id', async (req, res) => {
  const db = req.db;
  const id = req.params.id;
  const c = await db.prepare('SELECT code FROM coupons WHERE id = ?').get(id);
  await db.prepare('DELETE FROM coupons WHERE id = ?').run(id);
  if (c) await logActivity(db, req.session.adminId, req.session.adminUsername, 'حذف كوبون', c.code);
  res.redirect('/admin/coupons');
});
