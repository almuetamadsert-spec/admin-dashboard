 ...
 */
async function sendToMerchants(db, opts) {
...
/**
 * إشعار للتجار عند إنشاء طلب جديد (Push عبر OneSignal).
 * استهداف التاجر: إن وُجد merchantId يُرسل الإشعار لذلك التاجر فقط؛ وإلا لتجار نفس المدينة.
 * @param {object} db - قاعدة البيانات
 * @param {string} orderNumber - رقم الطلب
 * @param {number} totalAmount - المبلغ الإجمالي
 * @param {object} opts - اختياري: { merchantId } لاستهداف تاجر معيّن، { cityId } لتجار المدينة، { customerName, customerPhone }
 */
async function notifyMerchantsNewOrder(db, orderNumber, totalAmount, opts = {}) {
  let sql = "SELECT onesignal_player_id FROM merchants WHERE is_active = 1 AND onesignal_player_id IS NOT NULL AND TRIM(onesignal_player_id) != ''";
  const params = [];
  if (opts.merchantId != null && opts.merchantId !== '') {
    sql += " AND id = ?";
    params.push(opts.merchantId);
  } else if (opts.cityId != null) {
    sql += " AND city_id = ?";
    params.push(opts.cityId);
  }
  const merchants = await db.prepare(sql).all(...params);
  const subscriptionIds = merchants.map((m) => (m.onesignal_player_id || '').trim()).filter(Boolean);
  if (subscriptionIds.length === 0) return { ok: true };
